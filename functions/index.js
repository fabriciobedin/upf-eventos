...
exports.addParticipante = functions.https.onRequest(async (req, res) => {
  ...
  const eventosRef = admin.firestore().collection('Eventos');

  await eventosRef.doc(eventoId)
...
async function enviarCracha(evento,participante){
  const codigo = await criarPdf(participante,evento);
  ...
  async function criarPdf(participante,evento) {
    ...
    const storageCracha = new Storage();
    const bucket = storageCracha.bucket('upf-eventos.appspot.com');
    ...
    const qrcode = await createQrCode(codigo)
    var bufferCode = new Buffer(qrcode.split(',')[1] ||'', 'base64');
    doc.image(bufferCode, 10, 130, {height: 120});
  ...
  exports.pdf = functions.https.onRequest(async (req, res) => {
    ...
    var bufferCode = new Buffer(qrcode.split(',')[1] ||'', 'base64');
    doc.image(bufferCode, 10, 110, { height: 120 });
...
exports.buscarParticipante = functions.https.onRequest(async (req, res) => {
  ...
  const data = await buscarParticipante(participanteId);
  res.send(data);
});
